% TODO/NOTE: When introducing different parts of our model architecture, refer each of them as trying to solve one of the issues cited above.
% TODO/NOTE: So the 0.50 order variation score suggest that there is a balanced variability in ordering of actions suggesting that maybe the timestamp can tell us some info about which action is currently happening, this can further justify the use of LSTMs and the increase in performance & accuracy due to them.
% TODO/NOTE: When discussing the results talk about the temporal context window of each model.
% TODO/NOTE: Talk also about the fact that is it is fixed and can’t be changed.
% TODO/NOTE: Say that if we were able to change it we would set it the the minimum duration of some action or the minimum average as otherwise we would potentially lose and not be able to detect some classes.
...
\todo[inline]{- Reference the figure and each sub figure when describing the model.- Add a figure description that name each sub figure (a), (b), etc.}
